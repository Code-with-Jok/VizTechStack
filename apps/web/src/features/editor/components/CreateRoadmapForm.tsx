...
import type { CreateRoadmapFormData } from '../types';
...
export function CreateRoadmapForm() {
    ...
    const { createRoadmap, loading, errors, clearError } = useCreateRoadmap({
        onCompleted: (data) => {
            // Redirect to editor after successful creation
            router.push(`/admin/roadmaps/${data.slug}/edit`);
        },
    });
    ...
    const handleInputChange = (field: keyof CreateRoadmapFormData, value: string) => {
