...
import { ProgressTracker } from '@/features/progress/components';
import { useRoadmapBySlug, useProgressForRoadmap } from '../hooks';
...
export function RoadmapViewer({ slug, onNodeClick, showProgressTracker = true }: RoadmapViewerProps) {
    ...
    const { progressMap } = useProgressForRoadmap(
        roadmapId || '',
        !isSignedIn || !roadmapId
    );
