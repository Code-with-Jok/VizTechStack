...
export const TopicNode = memo(({ data, selected }: NodeProps) => {
  return (
    <div
      className={`px-4 py-3 shadow-xl rounded-xl border-2 transition-all duration-300 min-w-[150px] text-center ${
        selected
    ...
    >
      ...
      {/* Decorative element */}
      <div className="absolute -inset-0.5x bg-linear-to-r from-purple-500 to-indigo-500 rounded-xl opacity-0 hover:opacity-10 transition-opacity pointer-events-none" />
    </div>
  );
});
