...
async function generateFeature(featureName: string) {
    ...
    try {
        ...
        console.error(`❌ Feature "${featureName}" already exists at ${featurePath}`)
        process.exit(1)
    } catch {
...
`
    ...
    console.log(`  4. Export components/hooks from their respective index.ts files`)
...
async function main() {
    ...
    if (!/^[a-z]+(-[a-z]+)*$/.test(featureName)) {
        console.error('❌ Error: Feature name must be in kebab-case (e.g., user-profile)')
        process.exit(1)
    }
