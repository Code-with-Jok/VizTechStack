...
async function main() {
    ...
    try {
        ...
        for (const file of webStats.largestFiles.slice(0, 5)) {
            ...
            console.log(`    ${formatBytes(file.size).padStart(10)}  ${relativePath}`)
        }
        console.log()
    } catch {
        console.log('⚠️  Web build not found. Run `pnpm build --filter @viztechstack/web` first.')
        console.log()
    }
    ...
    try {
        ...
        console.log('⚠️  API build not found. Run `pnpm build --filter @viztechstack/api` first.')
        console.log()
    }
